...
high_quality={}
high_quality["inicountq"] = 1000
high_quality["inithreshq"] = 170
high_quality["inicountid"] = 30
high_quality["inithreshid"] = 140
...
low_quality["inicountid"] = 40
low_quality["inithreshid"] = 150
...
def write_in_xsl(question_results,file) :
    ...
    writer = pd.ExcelWriter('./model_4_results/'+file+'.xlsx', engine='xlsxwriter')
    ...
    while maxRadius > 0:
        ...
        while 5< minRadius < maxRadius:
            ...
            # if(circles is not None):
               # print(minRadius, maxRadius,circles.shape[1])
...
def get_code_and_questions(circles,quality_type) :
    circles = circles[0]
    if(len(circles)==totalCount+1):
        circles=circles[1:totalCount+1]
    ...
    while i < len(Question_Circles):
        Questions.append(sorted(Question_Circles[i:i + 4], key=itemgetter(0)))
        i = i + 4
...
def main() :
    files = os.listdir('./model_4_images/')

    jpg_files = [file for file in files if file.lower().endswith(".jpg")]

    for jpg_file in jpg_files:
        file_path = os.path.join("./model_4_images/",jpg_file)
       ...
       # print(Questions)
        ...
        # print("code:",str(jpg_file), code_results)
        # print("questions:", question_results)
        if circles is not None:
            for i in Questions:
                for j in i:
                    center = (j[0], j[1])
            ...
            file_path = os.path.join("./model_4_results/", jpg_file)
        # show_images([img])
